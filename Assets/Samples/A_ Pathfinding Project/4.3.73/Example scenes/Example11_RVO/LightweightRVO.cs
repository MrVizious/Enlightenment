...
namespace Pathfinding.Examples {
	...
	public partial class LightweightRVO : MonoBehaviour {
		...
		public void Start () {
			...
			var system = world.AddSystemManaged<LightweightRVOControlSystem>(new LightweightRVOControlSystem());
			simulationGroup.AddSystemToUpdateList(system);
			var renderSystem = world.AddSystemManaged<LightweightRVORenderSystem>(new LightweightRVORenderSystem {
				material = material,
		...
		public void Update () {
			var world = World.DefaultGameObjectInjectionWorld;
			var system = world.GetOrCreateSystemManaged<LightweightRVOControlSystem>();
			system.debug = debug;
		}
		...
		///
		/// It also relies on the <see cref="AIMoveSystem"/> and <see cref="RVOSystem"/>.
		/// </summary>
		[UpdateBefore(typeof(RVOSystem))]
		[UpdateInGroup(typeof(AIMovementSystemGroup))]
		[DisableAutoCreation]
		public partial class LightweightRVOControlSystem : SystemBase {
			...
			protected override void OnCreate () {
				entityQueryDirection = GetEntityQuery(
				...
				entityQueryControl = GetEntityQuery(
			...
			protected override void OnUpdate () {
				Dependency = new AlignAgentWithMovementDirectionJob {
					deltaTime = SystemAPI.Time.DeltaTime,
					rotationSpeed = 5,
				}.ScheduleParallel(entityQueryDirection, Dependency);

				Dependency = new JobControlAgents {
					deltaTime = SystemAPI.Time.DeltaTime,
					debug = debug,
				}.Schedule(entityQueryControl, Dependency);
			}
...
#else
namespace Pathfinding.Examples {
	[HelpURL("http://arongranberg.com/astar/documentation/beta/class_pathfinding_1_1_examples_1_1_lightweight_r_v_o.php")]
